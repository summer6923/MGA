...
class Server:
    ...
    async def process_client_info(self, client_id, sid):
        ...
        if client_id not in self.training_clients:
            logging.warning(
                "[%s] Ignoring a late or duplicate report from client #%d.",
                self,
                client_id,
            )
        ...
        self.training_clients.pop(client_id, None)

        if client_info[2]["sid"] in self.training_sids:
