...
class Trainer(base.Trainer):
    ...
    def get_avg_weights(self,config):
        ...
        for i in self.clients_unrequesting_deletion:
            try:
                ...
                if self.avg_mode == "uniform":
                    weight = 1.0
                else:
                    weight = float(pth_rec[i])
                    if weight <= 0:
                        continue
        ...
        keys = model_dicts[0].keys()
        for key in keys:
            averaged_dict[key] = sum(
                model_dict[key] * weight
        ...
        dist_ref_random_lst = []
        for _ in range(10):
            dist_ref_random_lst.append(Utils.get_distance(self.model_ref, self.origin_model).cpu())    

        self.threshold = np.mean(dist_ref_random_lst) / 3
        return self.model_ref
    ...
    def _update_pth_rec(self):
        ...
        rec_path = "pth_rec.npy"
        lock_path = rec_path + ".lock"
        with open(lock_path, "w") as lock_file:
            fcntl.flock(lock_file, fcntl.LOCK_EX)
            try:
                try:
                    pth_rec = np.load(rec_path)
                except (FileNotFoundError, ValueError, EOFError):
                    pth_rec = np.zeros(self.total_clients + 1)

                if pth_rec.shape[0] <= self.client_id:
                    expanded = np.zeros(self.total_clients + 1)
                    expanded[: pth_rec.shape[0]] = pth_rec
                    pth_rec = expanded

                pth_rec[self.client_id] = self.current_round
                tmp_path = rec_path + f".{os.getpid()}.tmp"
                np.save(tmp_path, pth_rec)
                os.replace(tmp_path + ".npy", rec_path)
            finally:
    ...
    def train_model(self, config, trainset, sampler, unlearning, **kwargs):
        ...
        if config.get("local_lr") is not None:
            for param_group in self.optimizer.param_groups:
                param_group["lr"] = float(config["local_lr"])
        ...
        for self.current_epoch in range(1, total_epochs + 1):
            ...
            for batch_id, (examples, labels) in enumerate(self.train_loader):
                ...
                if unlearning:
                    ...
                    with torch.no_grad():
                        distance = Utils.get_distance(self.model, self.model_ref)
                        if distance > self.threshold:
                            dist_vec = nn.utils.parameters_to_vector(self.model.parameters()) - nn.utils.parameters_to_vector(self.model_ref.parameters())
                            dist_vec = dist_vec/torch.norm(dist_vec)*np.sqrt(self.threshold)


                            proj_vec = (nn.utils.parameters_to_vector(self.model_ref.parameters()) + dist_vec)
                            nn.utils.vector_to_parameters(proj_vec, self.model.parameters())
                            distance = Utils.get_distance(self.model, self.model_ref)
                    
                    distance_ref_party_0 = Utils.get_distance(self.model, last_model)
    ...
    def train(self, trainset, sampler, unlearning=0,**kwargs) -> float:
        ...
        if kwargs.get("local_lr") is not None:
            config["local_lr"] = float(kwargs.pop("local_lr"))
