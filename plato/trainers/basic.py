...
class Trainer(base.Trainer):
    ...
    def __init__(self, model=None, callbacks=None):
        ...
        self.model_path = Config().params["model_path"]
        self.total_clients = Config().clients.total_clients
        self.clients_requesting_deletion = Config.clients.clients_requesting_deletion
        self.clients_unrequesting_deletion = []
        for i in range(1, self.total_clients + 1):
            if i not in self.clients_requesting_deletion:
                self.clients_unrequesting_deletion.append(i)
        self.model_ref = copy.deepcopy(self.model)
        self.clip_grad = 5
        self.threshold = float("inf")
        self.distance_threshold = 2.2
        self.model_name = Config().trainer.model_name
        self.unlearning_negate_loss = (
            Config().trainer.unlearning_negate_loss
            if hasattr(Config().trainer, "unlearning_negate_loss")
            else True
        )
        self.avg_mode = (
            Config().trainer.avg_mode
            if hasattr(Config().trainer, "avg_mode")
            else "weighted_pth_rec"
        )
        self.unlearning_epochs = (
            Config().trainer.unlearning_epochs
            if hasattr(Config().trainer, "unlearning_epochs")
            else Config().trainer.epochs
        )

    def get_avg_weights(self,config):

        model_dicts = []
        weights = []
        missing_reference_clients = []
        aggregation_counts_path = "client_aggregation_counts.npy"
        try:
            aggregation_counts = np.load(aggregation_counts_path)
        except (FileNotFoundError, ValueError, EOFError) as exc:
            raise RuntimeError(
                "Server aggregation counts are unavailable; weighted reference "
                "cannot be constructed."
            ) from exc

        for i in self.clients_unrequesting_deletion:
            aggregation_count = (
                float(aggregation_counts[i])
                if i < aggregation_counts.shape[0]
                else 0.0
            )
            if aggregation_count <= 0:
                continue
            try:
                filename = f"{self.model_path}/{self.model_name}_{i}.pth"
                state_dict = torch.load(filename, map_location="cpu")
                weight = 1.0 if self.avg_mode == "uniform" else aggregation_count
                model_dicts.append(state_dict)
                weights.append(weight)
            except Exception as exc:
                missing_reference_clients.append(i)
                logging.warning(
                    "[Trainer] Missing reference checkpoint for participated client %s: %s",
                    i,
                    exc,
                )

        if (
            bool(getattr(Config().trainer, "strict_reference_coverage", False))
            and missing_reference_clients
        ):
            raise RuntimeError(
                "Incomplete reference coverage for participated clients: "
                + ",".join(str(i) for i in sorted(set(missing_reference_clients)))
            )

        if not model_dicts:
            raise RuntimeError(
                "No retained client with a confirmed server aggregation is available "
                "for the MGA reference."
            )

        base = sum(weights)
        averaged_dict = {}
        for key in model_dicts[0].keys():
            params = [model_dict[key] for model_dict in model_dicts]
            if torch.is_floating_point(params[0]):
                averaged_dict[key] = sum(
                    value * weight for value, weight in zip(params, weights)
                ) / base
            else:
                averaged_dict[key] = (
                    sum(
                        value.to(dtype=torch.float64) * weight
                        for value, weight in zip(params, weights)
                    )
                    / base
                ).round().to(dtype=params[0].dtype)

        self.model_ref.load_state_dict(averaged_dict)
        self.model_ref.to(self.device)
        logging.info(
            "[Trainer] Reference mode=%s clients=%s total_weight=%.3f "
            "min_weight=%.3f max_weight=%.3f.",
            self.avg_mode,
            len(weights),
            float(base),
            float(min(weights)),
        ...
        self.threshold = float(
            Utils.get_distance(self.model, self.model_ref).detach().cpu().item()
        )
        logging.info(
            "[Trainer] MGA projection radius anchored to the received "
            "pre-unlearning model: squared_radius=%.8f.",
            self.threshold,
        )
    ...
    def save_model(self, filename=None, location=None):
        ...
        self._update_pth_rec()

    def _update_pth_rec(self):
        """Record the latest saved round without corrupting shared pth_rec.npy."""
        from plato.utils.pth_rec import update_pth_rec
    ...
    def train_process(self, config, trainset, sampler, unlearning,**kwargs):
        """
        ...
        """
        self.mga_reverse_result = None
        context = getattr(self, "mga_context", None)
        if context and bool(unlearning) != (context["phase"] == "unlearn"):
            raise ValueError("MGA dispatch phase and trainer operation disagree.")
        try:
            self.train_model(config, trainset, sampler.get(), unlearning, **kwargs)
        ...
        if "max_concurrency" in config:
            ...
            filename = f"{model_name}_{self.client_id}.pth"

            self.save_model(filename)

        if context:
            evidence = {key: value for key, value in context.items() if key != "execution_file"}
            evidence.update(
                reverse=self.mga_reverse_result,
                data_reads=int(getattr(trainset, "read_count", 0)),
                poison_reads=int(getattr(trainset, "poison_read_count", 0)),
            )
            if evidence["data_reads"] < 1:
                raise ValueError("No training data were consumed in this MGA dispatch.")
            ...
            logging.info("MGA_EXECUTION phase=%s client=%s data_reads=%s poison_reads=%s",
                         context["phase"], self.client_id, evidence["data_reads"], evidence["poison_reads"])
        elif hasattr(trainset, "poison_read_count"):
            logging.info("BACKDOOR_EXECUTION client=%s data_reads=%s poison_reads=%s",
                         self.client_id, trainset.read_count, trainset.poison_read_count)
    ...
    def train_model(self, config, trainset, sampler, unlearning, **kwargs):
        ...
        if unlearning and context and context["phase"] == "unlearn":
            ...
            loader = self.get_train_loader(
                int(config.get("unlearning_batch_size", config["batch_size"])), trainset, sampler)
            self.mga_reverse_result = reverse_optimize(
                self.model, self.mga_reference, loader, self.get_loss_criterion(),
                int(context["iterations"]), float(context["lr"]),
                float(context["radius"]), self.device,
            )
            self.run_history.update_metric("train_loss", self.mga_reverse_result["loss_after"])
            logging.info("MGA_REVERSE_COMPLETE client=%s %s", self.client_id,
                         json.dumps(self.mga_reverse_result, allow_nan=False))
            return
        batch_size = (
            int(config.get("unlearning_batch_size", config["batch_size"]))
            if unlearning
        ...
        if unlearning:
            total_epochs = int(config.get("unlearning_epochs", self.unlearning_epochs))
            logging.info(
                "[%s] Reverse optimization unlearning_epochs (E) = %s.",
                self,
                total_epochs,
            )
            if total_epochs <= 0:
                logging.info(
                    "[%s] Skipping reverse optimization because dynamic E <= 0.",
                    self,
                )
        ...
        use_mirror_reference = bool(
            unlearning and getattr(Config().trainer, "use_mirror_reference", True)
        )
        if use_mirror_reference:
            self.get_avg_weights(config)
            logging.info(
                "[%s] MGA reference prepared once; threshold=%.8f, mode=%s.",
                self,
        ...
        for self.current_epoch in range(1, total_epochs + 1):
            flag = False
            epoch_forget_batches = 0
            epoch_forget_loss_sum = 0.0
            ...
            for batch_id, (examples, labels) in enumerate(self.train_loader):
                ...
                if unlearning:
                    ...
                    if self.unlearning_negate_loss:
                        loss = -loss
                    self._loss_tracker.update(loss, labels.size(0))
                    ...
                    epoch_forget_loss_sum += float(raw_forget_loss.item())
                    epoch_forget_batches += 1
                    
                    if not use_mirror_reference:
                        distance_ref_party_0 = Utils.get_distance(
            ...
            if unlearning:
                pre_projection_distance = (
                    float(Utils.get_distance(self.model, self.model_ref).item())
                    if use_mirror_reference
                    else float("nan")
                )
                if use_mirror_reference:
                    with torch.no_grad():
                        if pre_projection_distance > float(self.threshold):
                            model_vector = nn.utils.parameters_to_vector(
                                self.model.parameters()
                            )
                            reference_vector = nn.utils.parameters_to_vector(
                                self.model_ref.parameters()
                            )
                            direction = model_vector - reference_vector
                            direction_norm = torch.norm(direction)
                            if direction_norm > 0:
                                projected_vector = reference_vector + (
                                    direction / direction_norm
                                ) * np.sqrt(float(self.threshold))
                                nn.utils.vector_to_parameters(
                                    projected_vector, self.model.parameters()
                                )
                    post_projection_distance = float(
                        Utils.get_distance(self.model, self.model_ref).item()
                    )
                else:
                    post_projection_distance = float("nan")
                mean_forget_loss = (
                    epoch_forget_loss_sum / epoch_forget_batches
                    if epoch_forget_batches
                    else float("nan")
                )
                logging.info(
                    "[%s] UNLEARNING_EPOCH epoch=%s/%s batches=%s "
                    "forget_loss=%.8f pre_projection_distance=%.8f "
                    "post_projection_distance=%.8f.",
                    self,
                    self.current_epoch,
                    total_epochs,
                    epoch_forget_batches,
                    mean_forget_loss,
                    pre_projection_distance,
                    post_projection_distance,
                )
            ...
            if (
                ...
                # filename = f"{self.client_id}_newest.pth"
                # self.save_model(filename)
                self.model.to(self.device)
    ...
    def train(self, trainset, sampler, unlearning=0,**kwargs) -> float:
        ...
        if kwargs.get("local_epochs") is not None:
            config["epochs"] = int(kwargs.pop("local_epochs"))
        if kwargs.get("unlearning_epochs") is not None:
            config["unlearning_epochs"] = int(kwargs.pop("unlearning_epochs"))
        ...
        if "max_concurrency" in config:
            ...
            train_proc = mp.Process(
                ...
                args=(config, trainset, sampler, unlearning),
                kwargs=kwargs,
            )
            train_proc.start()
            train_proc.join()
            if train_proc.exitcode != 0:
                # An old model_<client>.pth may exist; it is not proof this job succeeded.
                raise ValueError(f"Training subprocess for client {self.client_id} failed with exit {train_proc.exitcode}.")
            ...
            tic = time.perf_counter()
            self.train_process(config, trainset, sampler, unlearning, **kwargs)
            toc = time.perf_counter()
    ...
    def test_process(self, config, testset, sampler=None, **kwargs):
        ...
        if "max_concurrency" in config:
            ...
            filename = f"{model_name}_{self.client_id}.acc"
            self.save_accuracy(accuracy, filename)
        else:
            return accuracy

    def backdoor_attack(self, x_test, y_test, target_label, poison_rate=0.1):
        """
        Backdoor ATK
        ...
        Args:
            ...
            target_label: The label to be used for the backdoor attack.
            poison_rate: The rate of poisoning in the test data.
            
        Returns:
            x_test_poisoned: The poisoned test data.
            y_test_poisoned: The poisoned test labels.
        """
        num_samples = int(len(x_test) * poison_rate)
        indices = torch.randperm(len(x_test))[:num_samples]
        
        x_test_poisoned = x_test
        y_test_poisoned = y_test
        
        x_test_poisoned[indices] = 1 - x_test[indices]
        y_test_poisoned[indices] = target_label
        
        return x_test_poisoned, y_test_poisoned
        ...
        if hasattr(Config().trainer, "max_concurrency"):
            ...
            try:
                ...
                filename = (
                    ...
                    f"{model_name}_{self.client_id}.acc"
                )
    ...
    def test_model(self, config, testset, sampler=None, **kwargs):
        ...
        # x_poison,y_poison = self.backdoor_attack(x_poison,y_poison,25,0.8)
        # testset = torch.utils.data.TensorDataset(x_poison,y_poison)
