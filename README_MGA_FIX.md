...
This patch fixes two observed defects in the available `knot.py` / nested Plato path:
...
Run from the `knot` directory using the environment recorded with the validation.
...
the paper. This patch does not publish to GitHub or claim clean-machine dependency
installation: the tested environment inherits the existing PyTorch installation.
